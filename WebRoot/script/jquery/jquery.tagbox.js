...
(function (jQuery) {
    jQuery.fn.tagBox = function (options) {
        var defaults = {
            ...
            maxTags : -1,
            maxTagsErr : function (max_tags) {
                alert("A maximum of " + max_tags + " tags can be added!");
            },
            beforeTagAdd : function (tag_to_add) {
            },
            afterTagAdd : function (added_tag) {
            },
            afterTagRemove : function(removed){
        ...
        $elements.each(function () {
            ...
            if (!options.readonly) {
                ...
                jQuery('.' + options.className + '-remove-' + uuid).live("click", function () {
                    ...
                    $tag_input_elem.val('');
                    remove_tag(tag_item);
                    options.afterTagRemove(tag_item);
                });
            }
            //  Methods
            ...
            function generate_tags_list(tags_list) {
                ...
                jQuery.each(tags_list, function (key, val) {
                    if (val != "") {
                        var remove_tag_link = (options.readonly) ? '' : '<a href="javascript:void(0)" class="' + options.className + '-remove ' + options.className + '-remove-' + uuid + '" title="' + options.removeTagTitle + '" rel="' + val + '">' + options.removeTagText + '</a>';
